...
void RT::FindChannell(WORD y)
{
  ...
  if(IsIconic(G->hWnd)){
    ...
    SendMessage(hMDI,WM_MDIACTIVATE,(WPARAM)G->hWnd,NULL);
  }
  G->UnlockGauge();
}
...
void RT::Draw(HDC hdc,RECT&rc,DCtype dct,RECT*rcUpd)
{
...
else dr.P.style=7|AS_DRAXIS|AS_NOTICK|AS_NODIGIT|AS_NOSNAP|
		((P_st==Left)?AS_LOW:AS_HIGH)|AS_VERT;
...
G->UnlockGauge();
SelectObject(hdc,hpDef);
Text(hdc,Torg+Rang+(R_left?-dr.tickP:Tlen+dr.tickP),rc.bottom-Rh-(int)(0.5*Ph)-bf,"X,m",TA_TOP|(R_left?TA_RIGHT:TA_LEFT));
...
MoveToEx(hdc,Torg+Rang,dr.rcG.top,NULL);
...
hpDef=hpSave;
DeleteObject(fntLab);
if(clr)for(unsigned i=0;i<NCLR;++i)DeleteObject(hpClr[i]);
