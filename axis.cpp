...
void Axis::trytick(int fact)
{
int maxt=style&0x00ff;
if(maxt<2)maxt=2;
...
inline int X(int l,int w){return dir?l:coord+w*tl;}
inline int Y(int l,int w){return dir?coord+w*tl:l;}
//inline void Mto(HDC hdc,int l,int w){MoveToEx(hdc,X(l,w),Y(l,w),NULL);}
//inline void Lto(HDC hdc,int l,int w){LineTo(hdc,X(l,w),Y(l,w));}
void Line(HDC hdc,int l1,int w1,int l2,int w2)
{
POINT line[2];
line[0].x=X(l1,w1);
line[0].y=Y(l1,w1);
...
int Axis::Draw(HDC hdc,RECT&rc,int tcklen)
{
...
int Range::d2scr(double x,int imin,int imax)
{
