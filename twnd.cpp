...
HLOCAL Window::hLast=NULL;

void Window::AtUnlock(){}
...
BOOL Window::WinProc(Msg&)
{
return FALSE;
}

BOOL Window::Command(WPARAM cmd)
{
switch(cmd){
	case CM_WSIZE:
	...
	case CM_TO_PRINT:
		...
		return TRUE;
	case CM_FONT:
		if(SelFont())InvalidateRect(hWnd,NULL,TRUE);
		return TRUE;
	case CM_LINEWIDTH:
		if(SelLW())InvalidateRect(hWnd,NULL,TRUE);
		return TRUE;
}
return FALSE;
}
...
static void WindowSize(HWND hWnd,int&width,int&height)
{
RECT rc;
HDC hdc=GetDC(hWnd);
GetClientRect(hWnd,&rc);
SetMapMode(hdc,MM_HIMETRIC);
DPtoLP(hdc,(POINT FAR*)(&rc),2);
ReleaseDC(hWnd,hdc);
height=-rc.bottom;
width=rc.right;
}
...
METAFILEPICT FAR *MP=(METAFILEPICT FAR*)GlobalLock(hMP);
MP->xExt=width;
MP->yExt=height;
...
struct METAFILEHEADER{
	 ...
	 DWORD   reserved;
	 WORD    checksum;
	 METAFILEHEADER(){key=0x9AC6CDD7L;hmf=0;reserved=0;}
	 void Check(){checksum=XOR((WORD*)this,10);}
};

void MetaPict::ToFile(char*file)
{
...
void Window::Create(MDICREATESTRUCT&cs)
{
	...
	case WM_COMMAND:
		switch(wParam){
			case IDOK:
				...
				if(!err){
					MessageBox(hDlg,"Wrong width",NULL,MB_OK);
					return TRUE;
				}
				h=GetDlgItemInt(hDlg,IDC_WS_H,&err,FALSE);
				if(!err){
					MessageBox(hDlg,"Wrong height",NULL,MB_OK);
					return TRUE;
				}
				SetWindowPos(hWnd,NULL,NULL,NULL,w,h,SWP_NOMOVE|SWP_NOZORDER);
				EndDialog(hDlg,TRUE);
			case IDCANCEL:
