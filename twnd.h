...
inline void SetWinHL(HWND w,HLOCAL h){SetWindowLong(w,0,(LONG)h);}
inline HLOCAL GetWinHL(HWND w){return (HLOCAL)GetWindowLong(w,0);}
...
struct Msg{
	...
	LPARAM lParam;
	LRESULT ret;
	Msg(UINT m,WPARAM w,LPARAM l){msg=m;wParam=w;lParam=l;ret=0;}
};

enum DCtype {WMF=0,Screen=-1,Printer=-2};
...
virtual void Draw(HDC hdc,RECT&rc,DCtype t,RECT*rcUpd=NULL)=0;
...
inline Window* Window::GetWindow(HWND hWnd)
{
return Window::LockWindow(GetWinHL(hWnd));
}

inline void* Window::operator new(unsigned bytes,unsigned add)
{
...
inline void Window::operator delete(void*)
{
LocalUnlock(hLast);
LocalFree(hLast);
hLast=NULL;
...
inline void Window::UnlockWindow()
{
AtUnlock();
if(Destroy && ( (LocalFlags(hThis)&LMEM_LOCKCOUNT) == 1 ) )delete this;
else LocalUnlock(hThis);
}

inline Window*Window::LockWindow(HLOCAL h)
{
