...
if(succ){
	...
	sprintf(bak,"%s%s._dt",E->Dir,E->Name);
	unlink(bak);
	MessageBox(hFrame,"Error writing experiment!",NULL,MB_OK);
}
E->Dec();
SetTitle();
}

int Gauge::Write(HFILE hDAT,FILE *hIXC)
{
  ...
  if(_hwrite(hDAT,(LPSTR)(val+start),l)!=l){
    //	MessageBox(hFrame,"Can't write file! Disk full?",NULL,MB_OK);
    return 1;
  }
  return 0;
}

void Gauge::AfterWrite(Experiment*E,long&Pos)
{
int i,j;
V0-=Zero_corr;
Zero_corr=0;
