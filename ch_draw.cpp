...
char* Uscale(int i,const char*unit,const char*suff)
{
	int l;
	UPrefix*p;
...
char* DrOpt::Tscale(int i)
{
	return Uscale(i,"s","");
}

char* DrOpt::Pscale(int i,char*unit)
{
	return Uscale(i,unit,"");
}

char* DrOpt::Iscale(int i,char*unit)
{
	return Uscale(i,unit,"*s");
}

void DrOpt::SetRect(RECT&rc)
{
...
void Gauge::Draw(HDC hdc,RECT&rc,DCtype t,RECT*rcU)
{
...
void Gauge::Plot(HDC hdc,DrOpt&dr)
{
int i;
...
if((dr.T.style&AM_DIGITS)==AS_DIGIT ||
	(dr.P.style&AM_DIGITS)==AS_DIGIT ||
	(dr.I.style&AM_DIGITS)==AS_DIGIT){
	fntLab=CreateFont(
...
if((dr.I.style&AM_AXIS)==AS_DRAXIS){
	...
	if(i>dr.rcG.top &&i<dr.rcG.bottom){
		MoveToEx(hdc,dr.rcG.left,i,NULL);
