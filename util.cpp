...
{SetDlgItemText(hDlg,ctrl,gcvt(val,5,buf));}
...
if(GetDlgItemText(hDlg,ctrl,buf,20)){
	...
	if(*eptr!=0){
		...
		MessageBox(hFrame,buf+21,NULL,MB_OK);
	}else{
		val=v;
	}
}
...
void cbs(char*dir)
{// enshures backslash at the end of dirname
int l=strlen(dir);
if(!l){
	lstrcpy(dir,".\\");
	return;
...
int DLG(int ID,DLGPROC Proc ,LPARAM lp)
{
int ret;
...
void AlignWindow(HWND hwnd,int t,int l,int b,int r,int howy,int howx) //how: -1 - left 0 center 1 right
{
RECT rc;
