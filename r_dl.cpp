...
typedef enum {Pre=0,A_B=1,Delay=2} ChType;
static enum CS {SF=0,LF=1,SO=2,LO=3} ChanSel;
...
switch(msg){
	case WM_INITDIALOG:{
		...
		if(!SendDlgItemMessage(hDlg,IDC_OPN_LIST,LB_GETCOUNT,0,0)){
			MessageBox(hFrame,"No experiments found",NULL,MB_ICONEXCLAMATION | MB_OK);
...
void SetInfo(HWND hDlg,char* exp)
{
int i,Ca,Cl,Cs,s,l;
char buf[80];
strcpy(buf,HeadDir);
...
for(i=0;i<30;i++){
   ...
   long start,final;
	long delay;
	ChType flag;
	if(LZRead(hFile, buf, 80)!=80 ||
		sscanf (buf, "%lf %lf %d %ld %hd %hd %hd %ld %ld",
			      &rate_A,
			 	  &rate_B,
			              (int*)&flag,
					 &delay,
...
inline short int swap2(short int v)
{
  return ((v>>8)&0xFF) | ((v&0xFF)<<8);
}

void reverse(int count,short int *dp)
{
...
void CalTab::read(FILE*fp)
{
int j;
int dummy_d;
char dummy_s[2];
for(j=0,len=0;j<30;j++){
	if(fscanf (fp, " | %d%d%lf%1s |"
			, &Tab[len].gnum
        	, &dummy_d
			, &Tab[len].fcal
        	, dummy_s
		  )!=4){
		MessageBox(hFrame,"Error in calibration file",NULL,MB_OK);
		  len=0;
		  return;
	}
...
double CalTab::calibr(int gauge)
{
int i;
for(i=0;i<len;i++){
...
MessageBox(hFrame,buf,NULL,MB_OK);
return 0;
}
