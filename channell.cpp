...
switch(M.msg){
	...
	case WM_CLOSE:
		if(MessageBox(hFrame,"Do You really want delete this channell?",
				"Close Channell",MB_YESNO|MB_ICONQUESTION|MB_DEFBUTTON2)!=IDYES){
	...
	case WM_SIZE:
		rcValid=FALSE;
		if(PolyP)delete PolyP;PolyP=NULL;
		if(PolyI)delete PolyI;PolyI=NULL;
		return FALSE;
	case WM_KEYDOWN:
		switch(M.wParam){
			case VK_LEFT:
