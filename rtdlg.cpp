...
void CheckLine(HWND hDlg,int ind,BOOL G_chk=TRUE)
{
...
void UnCheckLine(HWND hDlg,int ind)
{
...
inline void CMIN(double&n,double i){if(i<n)n=i;}
inline void CMAX(double&n,double i){if(i>n)n=i;}
...
void SetDlgItemUnit(HWND hDlg,int ID,double Val,Unit*U)
{
	int i;
	if(Val==0.0)i=0;
	else for(i=0;U[i].name;++i){
...
int GetDlgItemUnit(HWND hDlg,int ID,double&val,Unit*U)
{
	double w;
	char*p;
	int i;
...
int RT::ReadDlg(HWND hDlg)
{
	...
	else if(IsDlgButtonChecked(hDlg,IDC_RT_PR))P_st=Right;
	else P_st=None;
	return TRUE;
}

void RT::InitDlg(HWND hDlg)
{
	int i,j;
	...
	else SetDlgItemText(hDlg,IDC_RT_TMIN,"Auto");
	if(!T1_def)SetDlgItemUnit(hDlg,IDC_RT_TMAX,T1,UTime);
	else SetDlgItemText(hDlg,IDC_RT_TMAX,"Auto");
	if(!R0_def)SetDlgItemUnit(hDlg,IDC_RT_RMIN,R0,ULen);
	else SetDlgItemText(hDlg,IDC_RT_RMIN,"Auto");
	if(!R1_def)SetDlgItemUnit(hDlg,IDC_RT_RMAX,R1,ULen);
	else SetDlgItemText(hDlg,IDC_RT_RMAX,"Auto");
	{for(GaugeIterator G;G;++G){
		G->displayed=FALSE;
		G->checked=FALSE;
	}}
...
switch(msg){
	...
	case WM_COMMAND:{
		...
		switch(idCtl){
			...
			case IDOK:{
				if(!Rt->ReadDlg(hDlg)){
					j=MessageBox(hFrame,"No channels selected!",NULL,MB_RETRYCANCEL);
					if(j==IDCANCEL)EndDialog(hDlg,0);
				}else{
					if(!Rt->hWnd)CreateRT(Rt);
					else InvalidateRect(Rt->hWnd,NULL,TRUE);
					Rt->UnlockWindow();
					Rt=NULL;
					EndDialog(hDlg,1);
				}
