...
MoveToEx(hdc,rcCut.left,rcCut.bottom,NULL);
...
else if(hResize==-1)rcCut.left=pt.x;
if(vResize==1)rcCut.top=pt.y;
else if(vResize==-1)rcCut.bottom=pt.y;
DrawCut();
Window *W=Window::GetWindow(hCapWin);
CutWnd *CW=dynamic_cast<CutWnd*>(W);
...
inline void Handle(HDC hdc,int x,int y)
{Rectangle(hdc,x-5,y-5,x+5,y+5);}

void CutWnd::DrawCutRect(HDC hdc)
{
	RECT rcCut;
	GetCutRC(rcCut);
