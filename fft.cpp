...
static double *fft_cos=NULL;
static int FFT_len=0;

static void FFT_prepare(int n)
{
int i;
double PI_n;
if(FFT_len==n)return;
...
// Входной массив - val содержит len чисел,
//  расположенных с интервалом delt.
// Выходные массивы freq_re и freq_im будут содержать
//  по len/2 чисел следующим образом:
//		freq_re[0] - действительная часть 0-го коэфф. Фурье
//		freq_im[0] - действительная часть len-го коэфф. Фурье
//		freq_re[i],freq_im[i] i=1...len-1 - i-й коэфф. Фурье
void ft4(double*val,int delt,double*freq_re,double*freq_im);
void fft(double*val,int delt,double*freq_re,double*freq_im,int len){
if(len==4){ft4(val,delt,freq_re,freq_im);return;}
int n=len/2;
int n2=n/2;
int n4=n2/2;
int i;
...
// теперь массивы содержат:
// Коэффициенты Фурье чётных эл-тов:
//  freq_re[0],freq_im[0] - 0-й и n2-й
//  freq_re[i],freq_im[i] i=1...n2-1 - i-й
// Коэффициенты Фурье нечётных эл-тов
//  freq_re[n2],freq_im[n2] - 0-й и n2-й
//  freq_re[i+n2],freq_im[i+n2] i=1...n2-1 - i-й
{double x;
// Преобразование коэффициентов, не имеющих мнимой части
// freq[0]=freq[0]+freq[n/2]   мнимой части нет
// freq[n]=freq[0]-freq[n/2] , но freq[n] хранится в freq_im[0]
...
for(i=1;i<n4;++i){
// пересчёт коэффициентов(в комплексном виде) i=1...n/4-1
...
{
// пересчёт оставшихся коэффициентов(в комплексном виде) i=n/4
// freq[i]  =     freq[i]    +     coeff[i]   *freq[n2+i]
// freq[n-i]=conj(freq[i])   -conj(coeff[i]   *freq[n2+i])
